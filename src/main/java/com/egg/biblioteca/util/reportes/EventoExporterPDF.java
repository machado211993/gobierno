...
public class EventoExporterPDF {

    private final List<Evento> listarEventos;

    public EventoExporterPDF(List<Evento> listarEventos) { //inicializa el listado
        super();
    ...
    private void escribirDatosDeLaTabla(PdfPTable tabla) { //trae los datos de la base de datos 
        for (Evento evento : listarEventos) {

           
            tabla.addCell(evento.getNombreEvento());


            tabla.addCell(String.valueOf(evento.getAltaEvento()));
    ...
    public void exportar(HttpServletResponse response) throws IOException {
        try (Document documento = new Document(PageSize.A4)) {
            ...
            Paragraph titulo = new Paragraph("Lista de Eventos", fuente);
