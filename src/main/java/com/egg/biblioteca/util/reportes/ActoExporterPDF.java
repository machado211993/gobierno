...
public class ActoExporterPDF {

    private final List<Acto> listarActos;

    public ActoExporterPDF(List<Acto> listarActos) { // inicializa el listado
        super();
        this.listarActos = listarActos;
    ...
    private void escribirCabeceraDeLaTabla(PdfPTable tabla) { // crea las cabeceras de la tabla
        ...
        celda.setPhrase(new Phrase("ALTA ACTO", fuente));
        tabla.addCell(celda);

    }

    private void escribirDatosDeLaTabla(PdfPTable tabla) { // trae los datos de la base de datos
        for (Acto acto : listarActos) {

            tabla.addCell(acto.getNombreActo());

            tabla.addCell(String.valueOf(acto.getAltaActo()));
