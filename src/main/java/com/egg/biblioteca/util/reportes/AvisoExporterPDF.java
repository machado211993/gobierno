...
public class AvisoExporterPDF {

    private final List<Aviso> listarAvisos;

    public AvisoExporterPDF(List<Aviso> listarAvisos) { // inicializa el listado
        super();
        this.listarAvisos = listarAvisos;
    ...
    private void escribirCabeceraDeLaTabla(PdfPTable tabla) { // crea las cabeceras de la tabla
        ...
        celda.setPhrase(new Phrase("ALTA AVISO", fuente));
        tabla.addCell(celda);

    }

    private void escribirDatosDeLaTabla(PdfPTable tabla) { // trae los datos de la base de datos
        for (Aviso aviso : listarAvisos) {

            tabla.addCell(aviso.getNombreAviso());

            tabla.addCell(String.valueOf(aviso.getAltaAviso()));
    ...
    public void exportar(HttpServletResponse response) throws IOException {
        try (Document documento = new Document(PageSize.A4)) {
            ...
            Paragraph titulo = new Paragraph("Lista de Avisos", fuente);
